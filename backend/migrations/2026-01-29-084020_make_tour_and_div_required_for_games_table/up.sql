...
-- *removing these only because they are dependent on the games table
-- readding them exactly as before directly afterwards
...
CREATE TABLE games (
    ...
    tournamentid UUID NOT NULL REFERENCES tournaments(tid),  -- *now required* - What Tournament is this game for
    divisionid UUID NOT NULL REFERENCES divisions(did),      -- *now required* - What Division
...
-- readding tables exactly as they were before:
