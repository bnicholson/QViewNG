...
// NOTE: Commenting this out for now. Why?: Domain assumes entity IDs will be received, however
// this code originally expected strings. This code will be reintroduced later on after the 
// QuizMachine client is able to receive, hold on to and use the IDs it needs to send with
// the quizevent so that QView understands which entities it is reporting on. All other code
// dependent on this code will also be put on hold.

// Another option would be to switch over to using only the Game's gid + quizevent event sequence
// number (within the scope of the Game) for identifying each quizevent's Game context.
...
// use crate::models::{common::BigId,roominfo::*,eventlog,roominfo};
// use crate::models::apicalllog::{apicalllog};
// use crate::models::quizevent::{self,QuizEvent};
// use crate::models::game::{self,Game,GameChangeset};
// use crate::database::Database;
// use utoipa::OpenApi;

// #[derive(OpenApi)]
// #[openapi(paths(index))]
// pub struct QuizEventDoc;
...
//             }
//             _ => {
//                 log::error!("{:?} {:?} Invalid parameter received in /quizevent api call {:?} ",module_path!(),line!(),
//                     pair);
//             }
//         }
//     }
...
//     let quizevent_psk = match std::env::var("QUIZEVENT_PSK") {
//         Ok(quizevent_psk) => {
//             quizevent_psk
//         },
//         Err(e) => {
...
//     sha1hasher.update(quizevent_psk);
...
// 	log::info!("{:?} {:?} QuizEvent: Org: {} BldgRoom: {}, Key: {}, Tk: {}, TN: {}, DN: {}, Room: {}, Round: {}, Question: {}, EventNumber: {} Name: {} Team: {} Quizzer: {}, EC: {}, Parm1: {} Parm2: {}, Timestamp: {}, Host: {}, MD5: {}, Nonce: {} {}, Sha1sum: {} Calculated sha1sum: {}",
...
//         // oh boy!!!
//         log::error!("{} {} /api/quizevent Sha1sums don't match {} {}",module_path!(), line!(), &eventlog_entry.s1s, rsltbase64);
...
//         path = "/quizevents",
//         responses(
//             (status = 200, description = "QuizEvents found successfully", body = QuizEvent),
//             (status = 404, description = "QuizEvents not found")
//         ),
//         params(
//             ("page" = Option<u64>, Query, description = "Page to read"),
//             ("page_size" = Option<u64>, Query, description = "How many QuizEvents to return")
//         )
//     )
// ]
// #[get("")]
