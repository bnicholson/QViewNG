...
/// room sequence number) for the round in a single paginated call.
#[get("/{id}/game-rows")]
async fn read_game_rows(
    ...
    match models::game::read_game_rows_of_round(&mut db, item_id.into_inner(), &params) {
...
async fn update(
    ...
    let round_id = item_id.into_inner();

    let round = match models::round::read(&mut conn, round_id) {
        Ok(r) => r,
...
async fn destroy(
    ...
    let result = models::round::delete(&mut conn, round_id);
