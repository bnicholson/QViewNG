...
pub fn count(db: &mut database::Connection) -> QueryResult<i64> {
    use crate::schema::projectors::dsl::*;
    projectors.count().get_result(db)
}
...
/// underlying projector row so the item remains recoverable. Returns (0, equipment_rows_affected).
...
pub fn delete(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    if equipment_dbo.projectorid.is_none() {
        return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's projectorid is none. Projector could not be deleted.").into()
        ));
    }
...
/// Purge the gear item: permanently remove both the equipment row and its projector row. Reads the
...
pub fn purge(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    let type_id = match equipment_dbo.projectorid {
        Some(v) => v,
        None => return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's projectorid is none. Projector could not be purged.").into()
        )),
    };

    // Remove the equipment row first (clears the FK reference), then the projector row.
    let equipment_purged = models::equipment_dbo::purge(db, equipment_id)?;
    let projector_purged = diesel::delete(projectors.filter(id.eq(type_id))).execute(db)?;
    Ok((projector_purged, equipment_purged))
}
