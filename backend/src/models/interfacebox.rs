...
/// underlying interfacebox row so the item remains recoverable. Returns (0, equipment_rows_affected).
...
pub fn delete(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    if equipment_dbo.interfaceboxid.is_none() {
        return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's interfaceboxid is none. InterfaceBox could not be deleted.").into()
        ));
    }
...
/// Purge the gear item: permanently remove both the equipment row and its interfacebox row. Reads
...
pub fn purge(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    let type_id = match equipment_dbo.interfaceboxid {
        Some(v) => v,
        None => return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's interfaceboxid is none. InterfaceBox could not be purged.").into()
        )),
    };

    // Remove the equipment row first (clears the FK reference), then the interfacebox row.
    let equipment_purged = models::equipment_dbo::purge(db, equipment_id)?;
    let interfacebox_purged = diesel::delete(interfaceboxes.filter(id.eq(type_id))).execute(db)?;
    Ok((interfacebox_purged, equipment_purged))
}
