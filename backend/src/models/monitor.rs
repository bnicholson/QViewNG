...
/// underlying monitor row so the item remains recoverable. Returns (0, equipment_rows_affected).
...
pub fn delete(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    if equipment_dbo.monitorid.is_none() {
        return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's monitorid is none. Monitor could not be deleted.").into()
        ));
    }
...
/// Purge the gear item: permanently remove both the equipment row and its monitor row. Reads the
...
pub fn purge(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    let type_id = match equipment_dbo.monitorid {
        Some(v) => v,
        None => return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's monitorid is none. Monitor could not be purged.").into()
        )),
    };

    // Remove the equipment row first (clears the FK reference), then the monitor row.
    let equipment_purged = models::equipment_dbo::purge(db, equipment_id)?;
    let monitor_purged = diesel::delete(monitors.filter(id.eq(type_id))).execute(db)?;
    Ok((monitor_purged, equipment_purged))
}
