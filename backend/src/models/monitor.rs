...
impl NewMonitorDbo {
    fn to_model(&self, new_equipment_dbo: NewEquipmentDbo) -> NewMonitor {
        NewMonitor {
            size: self.size.clone(),
            brand: self.brand.clone(),
            has_vga_out_port: self.has_vga_out_port,
...
pub fn covert_to_model_from_dbos(monitor_dbo: MonitorDbo, equipment_dbo: EquipmentDbo) -> Monitor {
    Monitor {
        id: monitor_dbo.id,
