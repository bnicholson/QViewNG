...
use crate::schema::games::dsl::{org,tournament,division,room,round,clientkey,ignore,ruleset};
...
pub fn create(db: &mut database::Connection, item: &GameChangeset) -> QueryResult<Game> {
...
pub fn read(db: &mut database::Connection, item_id: i64) -> QueryResult<Game> {
    use crate::schema::games::dsl::*;
    games.filter(gid.eq(item_id)).first::<Game>(db)
}

pub fn read_all(db: &mut database::Connection) -> QueryResult<Vec<Game>> {
    use crate::schema::games::dsl::*;
    games
        .order(gid)
        .limit(10)
...
pub fn update(db: &mut database::Connection, item_id: i64, item: &GameChangeset) -> QueryResult<Game> {
    use crate::schema::games::dsl::*;
