...
pub fn read(db: &mut database::Connection, powerstrip_id: i64) -> QueryResult<PowerStrip> {
    use crate::schema::powerstrips::dsl::*;
    use crate::schema::equipment::dsl::*;

    let equipment_dbo_result = 
        equipment
            .filter(crate::schema::equipment::dsl::powerstripid.eq(powerstrip_id))
    ...
    let powerstrip_dbo_result = 
        powerstrips
            .filter(crate::schema::powerstrips::dsl::id.eq(equipment_dbo.powerstripid.unwrap()))
...
/// underlying powerstrip row so the item remains recoverable. Returns (0, equipment_rows_affected).
...
pub fn delete(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    if equipment_dbo.powerstripid.is_none() {
        return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's powerstripid is none. PowerStrip could not be deleted.").into()
        ));
    }
...
/// Purge the gear item: permanently remove both the equipment row and its powerstrip row. Reads the
...
pub fn purge(db: &mut database::Connection, equipment_id: i64) -> QueryResult<(usize, usize)> {
    ...
    let type_id = match equipment_dbo.powerstripid {
        Some(v) => v,
        None => return Err(diesel::result::Error::QueryBuilderError(
            format!("Error: EquipmentDbo's powerstripid is none. PowerStrip could not be purged.").into()
        )),
    };

    // Remove the equipment row first (clears the FK reference), then the powerstrip row.
    let equipment_purged = models::equipment_dbo::purge(db, equipment_id)?;
    let powerstrip_purged = diesel::delete(powerstrips.filter(id.eq(type_id))).execute(db)?;
    Ok((powerstrip_purged, equipment_purged))
}
