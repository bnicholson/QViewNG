...
use backend::{database::Database, models::{tournament::Tournament, tournamentgroup_tournament::TournamentGroupTournament}};
...
#[actix_web::test]
async fn add_tournament_to_tournamentgroup_works() {
    ...
    let payload = fixtures::tournamentgroups::arrange_add_tournament_to_tournamentgroup_works_integration_test(&mut conn);
    ...
    let uri = format!("/api/tournamentgroups/{}/tournaments", payload.tournamentgroupid);
    let req = test::TestRequest::post()
        .uri(&uri)
        .set_json(&payload)
        .to_request();
    ...
    let body: EntityResponse<TournamentGroupTournament> = test::read_body_json(resp).await;
    assert_eq!(body.code, 201);
    assert_eq!(body.message, "");

    let tournamentgroup_tournament = body.data.unwrap();
    assert_eq!(tournamentgroup_tournament.tournamentgroupid, payload.tournamentgroupid);
    assert_eq!(tournamentgroup_tournament.tournamentid, payload.tournamentid);
}
...
async fn remove_tournament_from_tournamentgroup_works() {
    ...
    let (tg, tour) = 
        fixtures::tournamentgroups::arrange_remove_tournament_from_tournamentgroup_works_integration_test(&mut conn);
    ...
    let delete_uri = format!("/api/tournamentgroups/{}/tournaments/{}", tg.tgid, tour.tid);
    let delete_req = test::TestRequest::delete()
        .uri(&delete_uri)
        .to_request();
    ...
    let get_tours_uri = format!("/api/tournamentgroups/{}/tournaments?page={}&page_size={}", tg.tgid, PAGE_NUM, PAGE_SIZE);
