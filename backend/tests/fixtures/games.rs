...
pub fn seed_get_games_of_room(db: &mut database::Connection) -> (Game, Game) {  // return Game because it contains gid and roundid (and roomid)

    // Required: multiple rooms (2 minimum) with multiple games (2 minimum) in each room
    ...
    (game_2, game_4)  // returning all Games of Tournament 2
}

pub fn seed_get_games_of_team(db: &mut database::Connection) -> (Uuid, Game, Game) {  // return Game because it contains gid and roundid (and roomid)
