use backend::models::game::NewGame;
use diesel::prelude::*;
use uuid::Uuid;
...
pub fn seed_game_payload_dependencies(conn: &mut PgConnection) -> (Uuid,Uuid,Uuid,Uuid,Uuid,Uuid,Uuid) {
    let tournament = fixtures::tournaments::seed_tournament(conn);
...
// pub fn seed_game(conn: &mut PgConnection, did: Uuid) -> Game {
//     let new_game = new_game_one(did);
...
//     did: Uuid
// ) -> Vec<Game> {
//     let new_game_1 = new_game_one(did);
//     let new_game_2 = new_game_two(did);
//     let new_game_3 = new_game_three(did);
