 ...
 */
export interface TournamentTS extends Omit<Tournament, "fromdate" | "todate" | "tid" | "registration_open_date" | "registration_close_date"> {
  tid: string;
  fromdate: Dayjs | null;
  todate: Dayjs | null;
...
/**
 * Returns true if today falls within the tournament's inclusive registration window
 * (registration is open on and between the open and close dates; closed otherwise).
 */
export const isRegistrationOpen = (tournament: TournamentTS): boolean => {
  const { registration_open_date, registration_close_date } = tournament;
  if (!registration_open_date || !registration_close_date) return false;
  const today = dayjs();
  return !today.isBefore(registration_open_date, "day") && !today.isAfter(registration_close_date, "day");
};
...
const dateStringToDayjs = (tournament: Tournament): TournamentTS => {
  const { fromdate, todate, tid, registration_open_date, registration_close_date, ...rest } = tournament;
  return {
    ...rest,
    tid: String(tid),
    fromdate: dayjs(fromdate),
    todate: dayjs(todate),
    registration_open_date: registration_open_date ? dayjs(registration_open_date) : null,
    registration_close_date: registration_close_date ? dayjs(registration_close_date) : null,
  };
};
