...
export const MyProfileOverviewPage = () => {
  ...
  useEffect(() => {
    if (!auth.isAuthenticated || !auth.session?.userId) return

    fetch(`/api/users/${auth.session.userId}`, {
  ...
  }, [auth.isAuthenticated, auth.session?.userId])
  ...
  const save = async () => {
    ...
    const response = await fetch(`/api/users/${auth.session.userId}`, {
      method: 'PUT',
      headers: {
